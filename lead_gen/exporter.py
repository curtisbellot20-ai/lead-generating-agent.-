...
COL_WIDTHS = [35, 20, 18, 45, 35, 20, 18, 10, 18, 65]
...
def export_to_excel(leads: list[dict], query: str, location: str) -> str:
    ...
    for row_idx, lead in enumerate(leads, 4):
        row_fill = PatternFill(start_color=alt_gray, end_color=alt_gray, fill_type="solid") if row_idx % 2 == 0 else None
        values = [
            lead.get("name", ""),
    ...
    for lead in leads:
        sources[lead.get("source", "Unknown")] = sources.get(lead.get("source", "Unknown"), 0) + 1
