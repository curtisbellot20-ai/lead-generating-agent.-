...
from lead_gen import config, website_scraper
...
def _names_match(a: str, b: str) -> bool:
    ...
    return bool(wa and wb and len(wa & wb) / max(len(wa), len(wb)) >= 0.70)
...
def _ensure_fields(leads: list[dict]):
    """Guarantee all new fields exist so the exporter never KeyErrors."""
    defaults = {
        "decision_maker": "", "decision_maker_title": "",
        "linkedin": "", "twitter": "", "description": "",
        "registry_agent": "", "registry_member": "", "registry_status": "",
        "owner_confidence": "",
    }
    for lead in leads:
...
async def run_pipeline(params: dict | None = None):
    ...
    with Progress(
        ...
        task = progress.add_task("Scraping...", total=len(active_sources))

        if sources.get("google_maps"):
            progress.update(task, description="[bold]Google Maps[/bold] (primary, multi-query)...")
        ...
        if sources.get("yellow_pages"):
            progress.update(task, description="[bold]Yellow Pages[/bold]...")
            secondary_leads.extend(yellow_pages.scrape(query, location, max_per, max_pg))
            progress.advance(task)
        if sources.get("yelp"):
            progress.update(task, description="[bold]Yelp[/bold]...")
            secondary_leads.extend(yelp.scrape(query, location, max_per, max_pg))
            progress.advance(task)
        if sources.get("bbb"):
            progress.update(task, description="[bold]BBB[/bold]...")
            secondary_leads.extend(bbb.scrape(query, location, max_per, max_pg))
            progress.advance(task)
        if sources.get("angi"):
            progress.update(task, description="[bold]Angi[/bold]...")
            secondary_leads.extend(angi.scrape(query, location, max_per, max_pg))
            progress.advance(task)
        if sources.get("sunbiz"):
            progress.update(task, description="[bold]SunBiz[/bold]...")
            secondary_leads.extend(sunbiz.scrape(query, location, max_per, max_pg))
            progress.advance(task)
        if sources.get("chamber"):
            progress.update(task, description="[bold]Chamber of Commerce[/bold]...")
            secondary_leads.extend(chamber.scrape(config.CHAMBER_URLS, max_per))
            progress.advance(task)
        if sources.get("bni"):
            progress.update(task, description="[bold]BNI Chapters[/bold]...")
            secondary_leads.extend(bni.scrape(config.BNI_CHAPTER_URLS, max_per))
            progress.advance(task)

    # ── Step 3: Cross-reference ─────────────────────────────────
    console.rule("[bold]Step 3 — Cross-referencing sources[/bold]")
    if primary_leads:
        console.print(f"  Google Maps: [cyan]{len(primary_leads)}[/cyan] | Other: [cyan]{len(secondary_leads)}[/cyan]")
        all_leads = cross_reference(primary_leads, secondary_leads)
        all_leads = deduplicate(all_leads)
        console.print(f"  Result: [green]{len(all_leads)}[/green] leads\n")
    else:
        all_leads = deduplicate(primary_leads + secondary_leads)
        console.print(f"  [green]{len(all_leads)}[/green] leads\n")

    # ── Step 4: Google search enrichment ─────────────────────────
    console.rule("[bold]Step 4 — Google search enrichment[/bold]")
    all_leads = google_enrich(all_leads)
    console.print()

    # ── Step 5: Deep website scraping + AI extraction ──────────────
    console.rule("[bold]Step 5 — Deep website scraping (Playwright + AI)[/bold]")
    using_pw = await website_scraper.browser_start()
    if using_pw:
        console.print("  [green]✓ Playwright browser ready[/green]")
    else:
        console.print(
            "  [yellow]Playwright not installed — using requests fallback[/yellow]\n"
            "  [dim]Run: pip3 install playwright && playwright install chromium[/dim]"
        )
    try:
        all_leads = await find_emails(all_leads)
    finally:
        await website_scraper.browser_stop()
    console.print()

    # ── Step 6: SunBiz registry lookup ──────────────────────────
    console.rule("[bold]Step 6 — FL State Corporation Registry (SunBiz)[/bold]")
    all_leads = sunbiz.run_registry_lookup(all_leads)
    console.print()

    # ── Step 7: Claude AI outreach enrichment ──────────────────
    console.rule("[bold]Step 7 — Claude AI outreach notes[/bold]")
    _ensure_fields(all_leads)
    all_leads = enrich_leads(all_leads)
    console.print()

    # ── Step 8: Export ──────────────────────────────────────
    console.rule("[bold]Step 8 — Exporting[/bold]")
    ...
    owners_found  = sum(1 for l in all_leads if l.get("owner_name") and l["owner_name"] not in ("", "Owner"))
    registry_found = sum(1 for l in all_leads if l.get("registry_agent") or l.get("registry_member"))
    ...
    summary.add_row("Registry matches", f"{registry_found} / {len(all_leads)}")
